...
#define MAP std::unordered_map
    ...
    std::wstring wpat_expr = utf8_to_wstring(
        "'s|'t|'re|'ve|'m|'ll|'d| ?[" + unicode_letter_expr + "]+| ?[" + unicode_number_expr
        + "]+| ?[^\\s" + unicode_letter_expr + unicode_number_expr + "]+|\\s+(?!\\S)|\\s+");
    ...
    class Tokenizer
    {
      public:
                  ...
                  const std::string mask_token_str = "<mask>")
        {
            ...
            _bos_token_id = convert_token_to_id(bos_token_str);
            _eos_token_id = convert_token_to_id(eos_token_str);
            _pad_token_id = convert_token_to_id(pad_token_str);
        }
        // ~Encoder();
        std::vector<std::string> bpe(std::wstring token);
        std::vector<std::string> tokenize(std::string str);
        int64_t convert_token_to_id(std::string token);
        void padding_encode_single_with_special_tokens(std::string str,
                                                       size_t max_length,
    ...
    std::vector<std::string> Tokenizer::bpe(std::wstring token)
    {
        // bpe use wstring
    ...
    int64_t Tokenizer::convert_token_to_id(std::string token)
    {
    ...
    void Tokenizer::padding_encode_single_with_special_tokens(std::string str,
                                                              size_t max_length,
                                                              std::vector<int64_t>* input_ids,
                                                              std::vector<int64_t>* mask_ids)
    {
